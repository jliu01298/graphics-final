...
def second_pass( commands, num_frames ):
    ...
    for command in commands:
        if command['op'] == 'vary':
            ...
            d = (end_knob-start_knob)/(end_frame-start_frame)
    ...
    for frame in frames:
        ...
        for command in commands:
            ...
            if c == 'box':
                ...
                knob = command['knob']
                if knob:
                    theta = args[1] * (math.pi/180) * symbols[knob] 
                else:
            ...
            # end operation loop
        num0s = len(str(num_frames)) - len(str(i))

        ending = "0" * num0s + str(i)
        save_extension(screen, "./anim/"+name+ending)
        tmp = new_matrix()
        ident( tmp )
        stack = [ [x[:] for x in tmp] ]
        screen = new_screen()
