...
def draw_scanline(x0, z0, x1, z1, y, screen, zbuffer, color,  n1, n2, view, ambient, light, symbols, reflect):
    if x0 > x1:
        tx = x0
        tz = z0
        x0 = x1
    ...
    if (x1 - x0 + 1) != 0:
        d_n = [(n1[0]-n2[0])/(x1 - x0 + 1.0), (n1[1]-n2[1])/(x1 - x0 + 1.0), (n1[2]-n2[2])/(x1 - x0 + 1.0)]
    else:
        d_n = [0, 0, 0]
...
def scanline_convert(polygons, i, screen, zbuffer, color, norms, view, ambient, light, symbols, reflect):
    flip = False
    BOT = 0
    TOP = 2
    MID = 1
    ...
    while y <= int(points[TOP][1]):
        if ( not flip and y >= int(points[MID][1])):
            ...
            if distance1 != 0:
                dn1 = [(topV[0]-midV[0])/distance1, (topV[1]-midV[1])/distance1, (topV[2]-midV[2])/distance1]
            else:
                dn1 = [0, 0, 0]
            x1 = points[MID][0]
            z1 = points[MID][2]
        ...
        draw_scanline(int(x0), z0, int(x1), z1, y, screen, zbuffer, color, botV, botV1, view, ambient, light, symbols, reflect)
        x0+= dx0
        z0+= dz0
        x1+= dx1
        z1+= dz1
...
def create_norms(polygons):
    ...
    while point < len(polygons) - 2:
        ...
        for x in range(3):
            if str(polygons[point+x]) in norms:
                norms[str(polygons[point+x])][0]+=normal[0]
                norms[str(polygons[point+x])][1]+=normal[1]
                norms[str(polygons[point+x])][2]+=normal[2]
            else:
                norms[str(polygons[point+x])]=normal[:]
        point+= 3
    for key in norms:
        normalize(norms[key])
    return norms
...
def draw_polygons( polygons, screen, zbuffer, view, ambient, light, symbols, reflect):
    ...
    while point < len(polygons) - 2:
        ...
        if normal[2] > 0:
            ...
            scanline_convert(polygons, point, screen, zbuffer, color, norms, view, ambient, light, symbols, reflect)
